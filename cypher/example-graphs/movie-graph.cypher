//# :play movie-graph
//# Movie Graph
//# Pop-cultural connections between actors and
/*
The Movie Graph is a minimal graph application contianing actors and directors that are related through the movies they've collaborated on.

This guide will show you how to:
    Create: insert a movie data into the graph
    Find: retrieve individual movies and actors
    Query: discovery related actors and directors
...
CREATE
  ...
  (JTW)-[:ACTED_IN {roles: ['Lt. Col. Matthew Andrew Markinson']}]->
  (AFewGoodMen),
...
CREATE
  ...
  (AnnabellaS)-[:ACTED_IN {roles: ['Annie Collins-Nielsen']}]->
  (WhatDreamsMayCome),
...
CREATE
  ...
  (CloudAtlas),
  (HalleB)-
    [:ACTED_IN {roles: ['Luisa Rey', 'Jocasta Ayrs', 'Ovid', 'Meronym']}]->
  (CloudAtlas),
  (JimB)-
    [:ACTED_IN {
...
CREATE
  (VforVendetta:Movie
    {title: 'V for Vendetta', released: 2006, tagline: 'Freedom! Forever!'})
...
CREATE
  (SpeedRacer:Movie
    {title: 'Speed Racer', released: 2008, tagline: 'Speed has no limits'})
...
CREATE
  ...
  (Madonna)-[:ACTED_IN {roles: ['"All the Way" Mae Mordabito']}]->
  (ALeagueofTheirOwn),
...
CREATE
  ...
  (TheReplacements),
  (JamesThompson)-
    [:REVIEWED {summary: 'The coolest football movie ever', rating: 100}]->
  (TheReplacements),
  (AngelaScope)-[:REVIEWED {summary: 'Pretty funny at times', rating: 62}]->
  (TheReplacements),
  (JessicaThompson)-[:REVIEWED {summary: 'Dark, but compelling', rating: 85}]->
  (Unforgiven),
  (JessicaThompson)-
    [:REVIEWED {
        summary:
  ...
  (TheBirdcage),
  (JessicaThompson)-[:REVIEWED {summary: 'A solid romp', rating: 68}]->
  (TheDaVinciCode),
  (JamesThompson)-
    [:REVIEWED {summary: 'Fun, but a little far fetched', rating: 65}]->
  (TheDaVinciCode),
  (JessicaThompson)-[:REVIEWED {summary: 'You had me at Jerry', rating: 92}]->
  (JerryMaguire)
